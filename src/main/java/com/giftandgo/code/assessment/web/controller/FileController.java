...
public class FileController {
    ...
    public ResponseEntity<byte[]> processUsers(@Valid @RequestBody ProcessUsersRequest processUsersRequest) {
        ...
        List<UserRequestDTO> userRequestDTOS = fileParsingService.parseAndReadUsers(processUsersRequest.usersInBytes());
        userValidationService.validateUsers(userRequestDTOS);
        String outcomeJson = outcomeService.generateOutcomeData(userRequestDTOS);
