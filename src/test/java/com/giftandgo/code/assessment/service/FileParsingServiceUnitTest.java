...
public class FileParsingServiceUnitTest {
    ...
    public void givenFileInBytes_whenFileExists_thenReturnListOfUsers() {
        ...
        doReturn(userDTOS).when(fileParserService).processAndReadUsersFromFile(eq(readFile));

        List<UserDTO> resultUserDTOList = fileParserService.parseAndReadUsers(new byte[1]);

        assertEquals(userDTOS.size(), resultUserDTOList.size());
        assertEquals(userDTOS.get(0), resultUserDTOList.get(0));
    ...
    public void givenFile_whenContentIsCorrect_thenReturnListOfUsers() {
        doReturn(UserDTO.builder().build()).when(fileParserService).parseUserFromFile(anyString());

        ClassLoader classLoader = getClass().getClassLoader();
        File file = new File(classLoader.getResource("files/EntryFile.txt").getFile());

        List<UserDTO> userDTOS = fileParserService.processAndReadUsersFromFile(file);
    ...
    public void givenLineFromFile_whenUserIsValid_thenReturnCorrectUser() {
        ...
        UserDTO userDTO = fileParserService.parseUserFromFile(line);

        assertEquals("18148426-89e1-11ee-b9d1-0242ac120002", userDTO.uuid());
        assertEquals("1X1D14", userDTO.id());
        assertEquals("John Smith", userDTO.name());
        assertEquals("Likes Apricots", userDTO.likes());
        assertEquals("Rides A Bike", userDTO.transport());
        assertEquals("6.2", userDTO.averageSpeed());
        assertEquals("12.1", userDTO.topSpeed());
    }
